const Forfunction = (doTime = 1) => fnProcess => arg => {
  ...
  else return console.error(`the number of iterations is not Nature number`);
};
const Forfunction2 = (doTime = 1) => (fnProcess, arg) => {
  return arg === undefined
    ? Forfunction(doTime)(fnProcess)
    : Forfunction(doTime)(fnProcess)(arg);
};
/**
...
const invokeFor = function(doTimefn, fnProcess, arg) {
  if (arguments.length === 1) return Forfunction2(doTimefn(arg));
  else if (arguments.length === 2) return Forfunction(doTimefn(arg))(fnProcess);
